...
namespace MLcar.AI
{
    ...
    /// <summary>
    /// The carAgent will drive the inputs for the carController.
    /// </summary>
    public class carAgent : Agent, IInput
    {
            ...
            switch (Mode)
            {
                case AgentMode.Inferencing:
                        ...
                        && ((1 << hit.collider.gameObject.layer) & OutOfBoundsMask) > 0)
                    {
                        ...
                        car.Rigidbody.velocity = default;
        ...
        public override void CollectObservations()
        {
            ...
            var direction = (nextCollider.transform.position - car.transform.position).normalized;
            AddVectorObs(Vector3.Dot(car.Rigidbody.velocity.normalized, direction));
        ...
        public override void AgentAction(float[] vectorAction)
        {
            ...
            Vector3 direction = (nextCollider.transform.position - car.transform.position).normalized;
            float reward = Vector3.Dot(car.Rigidbody.velocity.normalized, direction);

            if (ShowRaycasts)
            {
                Debug.DrawRay(AgentSensorTransform.position, car.Rigidbody.velocity, Color.blue);
            }

            // Add rewards if the agent is heading in the right direction
            AddReward(reward * TowardsCheckpointReward);
            AddReward(car.LocalSpeed() * SpeedReward);
        }
